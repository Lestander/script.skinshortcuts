...
def kodiwalk(path, stringForce=False):
    json_query = xbmc.executeJSONRPC(
        '{"jsonrpc":"2.0","method":"Files.GetDirectory","params":{"directory":"%s","media":"files"},"id":1}' % str(
            path))
    json_query = unicode(json_query, 'utf-8', errors='ignore')
    json_response = simplejson.loads(json_query)
    files = []
    if json_response.has_key('result') and json_response['result'].has_key('files') and json_response['result'][
        'files'] is not None:
...
class LibraryFunctions():
    def __init__(self, *args, **kwargs):
        ...
        self.widgetProviders = [["service.library.data.provider", None, "Library Data Provider"],
                                ...
                                ["service.smartish.widgets", "Skin.HasSetting(enable.smartish.widgets)",
                                 "Smart(ish) Widgets"]]
    ...
    def loadAllLibrary(self):
        ...
        if self.loaded["upnp"][0] == False:
            json_query = xbmc.executeJSONRPC(
                '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetDirectory", "params": { "properties": ["title", "file", "thumbnail"], "directory": "upnp://", "media": "files" } }')
        ...
        if flat:
            ...
            for node in nodes:
                ...
                if "version" in node.attrib:
                    ...
                    if KODIVERSION != version and DATA.checkVersionEquivalency(version, node.attrib.get("condition"),
                                                                               "groupings") == False:
                        group += 1
                        continue
    ...
    def getNode(self, tree, number):
        ...
        for subnode in tree:
            ...
            if "version" in subnode.attrib:
                ...
                if KODIVERSION != version and DATA.checkVersionEquivalency(version, subnode.attrib.get("condition"),
                                                                           "groupings") == False:
                    number += 1
                    continue
    ...
    def buildNodeListing(self, nodes, flat):
        ...
        for node in nodes:
            ...
            if node.tag == "shortcut":
                shortcutItem = self._create(
                    [node.text, node.attrib.get("label"), node.attrib.get("type"), {"icon": node.attrib.get("icon")}])
            ...
            if node.tag == "node" and flat == False:
                returnList.append(self._create(
                    ["||NODE||" + str(count), node.attrib.get("label"), "", {"icon": "DefaultFolder.png"}]))
    ...
    def _create(self, item, allowOverrideLabel=True):
        ...
        DATA._clear_labelID()
        labelID = DATA._get_labelID(DATA.createNiceName(DATA.local(localLabel)[0], noNonLocalized=noNonLocalized),
                                    item[0], noNonLocalized=noNonLocalized)
        ...
        if thumbnail is not None:
            listitem = xbmcgui.ListItem(label=displayLabel, label2=displayLabel2, iconImage=displayIcon,
    ...
    def _get_icon_overrides(self, tree, item, content, setToDefault=True):
        ...
        for elem in tree.findall("icon"):
            if oldicon is None:
                if ("labelID" in elem.attrib and elem.attrib.get("labelID") == item.getProperty("tempLabelID")) or (
                        "image" in elem.attrib and elem.attrib.get("image") == item.getProperty("icon")):
                    # LabelID matched
                    if "grouping" in elem.attrib:
    ...
    def _parse_libraryNodes(self, library, type):
        ...
        for key in nodes:
            ...
            if nodes[key][3] == "folder":
                item = self._create(
                    ["%s%s" % (action, nodes[key][2]), nodes[key][0], nodes[key][3], {"icon": nodes[key][1]}])
            elif nodes[key][3] == "grouped":
                item = self._create(
                    ["%s%s" % (action, nodes[key][2]), nodes[key][0], nodes[key][3], {"icon": nodes[key][1]}])
            else:
                item = self._create(
                    ["ActivateWindow(%s,%s,return)" % (windowID, nodes[key][2]), nodes[key][0], nodes[key][3],
                     {"icon": nodes[key][1]}])
    ...
    def common(self):
        ...
        listitems.append(self._create(
            ["ActivateWindow(Videos,videodb://movies/titles/,return)", "342", "32034", {"icon": "DefaultMovies.png"}]))
        listitems.append(self._create(["ActivateWindow(Videos,videodb://tvshows/titles/,return)", "20343", "32034",
        ...
        if int(KODIVERSION) >= 18:
            ...
            listitems.append(
                self._create(["ActivateWindow(Music)", "10005", "32034", {"icon": "DefaultMusicAlbums.png"}]))
        listitems.append(self._create(["PlayerControl(PartyMode)", "589", "32034", {"icon": "DefaultMusicAlbums.png"}]))
        listitems.append(
            self._create(["PlayerControl(PartyMode(Video))", "32108", "32034", {"icon": "DefaultMusicVideos.png"}]))

        listitems.append(self._create(["ActivateWindow(Videos,videodb://musicvideos/titles/,return)", "20389", "32034",
        ...
        listitems.append(
            self._create(["ActivateWindow(Programs,Addons,return)", "10001", "32034", {"icon": "DefaultProgram.png"}]))
        ...
        if int(KODIVERSION) >= 16:
            listitems.append(
                self._create(["ActivateWindow(EventLog,events://,return)", "14111", "32034", {"icon": "Events.png"}]))
    ...
    def more(self):
        ...
        if (xbmc.getCondVisibility("System.Platform.Windows") or xbmc.getCondVisibility(
                "System.Platform.Linux")) and not xbmc.getCondVisibility("System.Platform.Linux.RaspberryPi"):
        ...
        listitems.append(self._create(
            ["AlarmClock(shutdowntimer,XBMC.Shutdown())", "19026", "32054", {"icon": "ShutdownTimer.png"}]))
        listitems.append(
            self._create(["CancelAlarm(shutdowntimer)", "20151", "32054", {"icon": "CancelShutdownTimer.png"}]))
        ...
        listitems.append(
            self._create(["UpdateLibrary(video,,true)", "32046", "32054", {"icon": "UpdateVideoLibrary.png"}]))
        listitems.append(
            self._create(["UpdateLibrary(music,,true)", "32047", "32054", {"icon": "UpdateMusicLibrary.png"}]))

        listitems.append(
            self._create(["CleanLibrary(video,true)", "32055", "32054", {"icon": "CleanVideoLibrary.png"}]))
        listitems.append(
            self._create(["CleanLibrary(music,true)", "32056", "32054", {"icon": "CleanMusicLibrary.png"}]))
    ...
    def settings(self):
        ...
        listitems.append(
            self._create(["ActivateWindow(ServiceSettings)", "14036", "10004", {"icon": "ServiceSettings.png"}]))
        listitems.append(
            self._create(["ActivateWindow(SystemSettings)", "13000", "10004", {"icon": "SystemSettings.png"}]))
        listitems.append(self._create(["ActivateWindow(SkinSettings)", "20077", "10004", {"icon": "SkinSettings.png"}]))

        if int(KODIVERSION) <= 16:
            listitems.append(
                self._create(["ActivateWindow(VideosSettings)", "3", "10004", {"icon": "VideoSettings.png"}]))
            listitems.append(
                self._create(["ActivateWindow(MusicSettings)", "2", "10004", {"icon": "MusicSettings.png"}]))
            listitems.append(
                self._create(["ActivateWindow(PicturesSettings)", "1", "10004", {"icon": "PictureSettings.png"}]))
            listitems.append(self._create(
                ["ActivateWindow(AppearanceSettings)", "480", "10004", {"icon": "AppearanceSettings.png"}]))
            listitems.append(
                self._create(["ActivateWindow(WeatherSettings)", "8", "10004", {"icon": "WeatherSettings.png"}]))
        else:
            listitems.append(
                self._create(["ActivateWindow(PlayerSettings)", "14200", "10004", {"icon": "PlayerSettings.png"}]))
            listitems.append(
                self._create(["ActivateWindow(LibrarySettings)", "14202", "10004", {"icon": "LibrarySettings.png"}]))
            listitems.append(self._create(
                ["ActivateWindow(InterfaceSettings)", "14206", "10004", {"icon": "InterfaceSettings.png"}]))
    ...
    def pvrlibrary(self):
        ...
        listitems.append(
            self._create(["ActivateWindow(TVRecordings)", "19163", "32017", {"icon": "DefaultTVShows.png"}]))
        listitems.append(self._create(["ActivateWindow(TVTimers)", "19040", "32017", {"icon": "DefaultTVShows.png"}]))
        if int(KODIVERSION) >= 17:
            listitems.append(
                self._create(["ActivateWindow(TVTimerRules)", "19138", "32017", {"icon": "DefaultTVShows.png"}]))
        ...
        listitems = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "PVR.GetChannels", "params": { "channelgroupid": "alltv", "properties": ["thumbnail", "channeltype", "hidden", "locked", "channel", "lastplayed"] } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)

        # Add all directories returned by the json query
        if json_response.has_key('result') and json_response['result'].has_key('channels') and json_response['result'][
            'channels'] is not None:
            for item in json_response['result']['channels']:
                listitems.append(self._create(
                    ["pvr-channel://" + str(item['channelid']), item['label'], "::SCRIPT::32076",
                     {"icon": "DefaultTVShows.png", "thumb": item["thumbnail"]}]))
        ...
        listitems = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "PVR.GetChannels", "params": { "channelgroupid": "allradio", "properties": ["thumbnail", "channeltype", "hidden", "locked", "channel", "lastplayed"] } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)
    ...
    def radiolibrary(self):
        ...
        # PVR
        listitems.append(
            self._create(["ActivateWindow(RadioChannels)", "19019", "32087", {"icon": "DefaultAudio.png"}]))
        listitems.append(self._create(["ActivateWindow(RadioGuide)", "22020", "32087", {"icon": "DefaultAudio.png"}]))
        listitems.append(
            self._create(["ActivateWindow(RadioRecordings)", "19163", "32087", {"icon": "DefaultAudio.png"}]))
        listitems.append(self._create(["ActivateWindow(RadioTimers)", "19040", "32087", {"icon": "DefaultAudio.png"}]))
        if int(KODIVERSION) >= 17:
            listitems.append(
                self._create(["ActivateWindow(RadioTimerRules)", "19138", "32087", {"icon": "DefaultAudio.png"}]))
    ...
    def librarysources(self):
        ...
        listitems = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetSources", "params": { "media": "video" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)

        # Add all directories returned by the json query
        if json_response.has_key('result') and json_response['result'].has_key('sources') and json_response['result'][
            'sources'] is not None:
            for item in json_response['result']['sources']:
                listitems.append(
                    self._create(["||SOURCE||" + item['file'], item['label'], "32069", {"icon": "DefaultFolder.png"}]))
        ...
        listitems = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetSources", "params": { "media": "music" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)
        ...
        if json_response.has_key('result') and json_response['result'].has_key('sources') and json_response['result'][
            ...
            for item in json_response['result']['sources']:
                listitems.append(
                    self._create(["||SOURCE||" + item['file'], item['label'], "32073", {"icon": "DefaultFolder.png"}]))
        ...
        listitems = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetSources", "params": { "media": "pictures" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)
        ...
        if json_response.has_key('result') and json_response['result'].has_key('sources') and json_response['result'][
            ...
            for item in json_response['result']['sources']:
                listitems.append(
                    self._create(["||SOURCE||" + item['file'], item['label'], "32089", {"icon": "DefaultFolder.png"}]))
    ...
    def playlists(self):
        audiolist = []
        videolist = []
        paths = [['special://videoplaylists/', '32004', 'Videos'], ['special://musicplaylists/', '32005', 'Music'],
                 ["special://skin/playlists/", '32059', None], ["special://skin/extras/", '32059', None]]
        for path in paths:
            count = 0
                ...
                try:
                    ...
                    if playlist.endswith('.xsp'):
                        ...
                        for line in xmldata.getiterator():
                            ...
                            if line.tag == "name" and mediaLibrary is not None:
                                ...
                                listitem.setProperty("action-show",
                                                     "ActivateWindow(" + mediaLibrary + "," + playlist + ",return)".encode(
                                                         'utf-8'))
                        ...
                        listitem = self._create(
                            ["::PLAYLIST>%s::" % (path[2]), name, path[1], {"icon": "DefaultPlaylist.png"}])
                        listitem.setProperty("action-play", "PlayMedia(" + playlist + ")")
                        listitem.setProperty("action-show",
                                             "ActivateWindow(%s,%s,return)".encode('utf-8') % (path[2], playlist))
                        listitem.setProperty("action-party", "PlayerControl(PartyMode(%s))" % (playlist))
    ...
    def addons(self):
        ...
        contenttypes = [("executable", executableItems), ("video", videoItems), ("audio", audioItems),
                        ("image", imageItems)]
        for contenttype, listitems in contenttypes:
            ...
            json_query = xbmc.executeJSONRPC(
                '{ "jsonrpc": "2.0", "id": 0, "method": "Addons.Getaddons", "params": { "content": "%s", "properties": ["name", "path", "thumbnail", "enabled"] } }' % contenttype)
            json_query = unicode(json_query, 'utf-8', errors='ignore')
            json_response = simplejson.loads(json_query)

            if json_response.has_key('result') and json_response['result'].has_key('addons') and \
                ...
                for item in json_response['result']['addons']:
                    if item['enabled'] == True:
                        ...
                        if item['thumbnail'] != "":
                            ...
                            thumb = None
                        listitem = self._create(
                            [path, item['name'], shortcutType, {"icon": "DefaultAddon.png", "thumb": thumb}])
                        ...
                        if item["type"] == "xbmc.python.pluginsource":
                            ...
                            for content in provides:
                                ...
                                contentData = {"video": ["::SCRIPT::32010", videoItems],
                                ...
                                if content in contentData:
                                    ...
                                    otherItem = self._create([path, item['name'] + "  >", contentData[content][0],
                                                              {"icon": "DefaultAddon.png", "thumb": thumb}])
    ...
    def widgets(self):
        ...
        for elem in elems:
            ...
            self.dictionaryGroupings["widgets-classic"].append(
                [elem.text, DATA.local(elem.attrib.get('label'))[2], widgetType, widgetPath, widgetIcon, widgetTarget])

            # Save widgets for button 312
            listitem = self._create(
                [elem.text, DATA.local(elem.attrib.get('label'))[2], "::SCRIPT::32099", {"icon": widgetIcon}])
    ...
    def explorer(self, history, location, label, thumbnail, itemType, isWidget=False):
        ...
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetDirectory", "params": { "properties": ["title", "file", "thumbnail", "episode", "showtitle", "season", "album", "artist", "imdbnumber", "firstaired", "mpaa", "trailer", "studio", "art"], "directory": "' + location + '", "media": "files" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)
        ...
        if json_response.has_key('result') and json_response['result'].has_key('files') and json_response['result'][
            ...
            for item in json_result:
                    ...
                    if item["filetype"] == "directory":
                        ...
                        listitem = self._create(
                            ["ActivateWindow(%s,%s,return)" % (windowID, item["file"]), altLabel, "",
                             {"icon": "DefaultFolder.png", "thumb": thumb}])
                ...
                # some special code for smart shortcuts in script.skin.helper.service
                    ...
                    if smartShortCutsData["type"] == "music" or smartShortCutsData["type"] == "artists" or \
                            smartShortCutsData["type"] == "albums" or smartShortCutsData["type"] == "songs":
                        listitem.setProperty("widgetTarget", "music")
                    else:
                        listitem.setProperty("widgetTarget", "videos")
                    ...
                    if item["filetype"] == "directory":
                        ...
                        if item["thumbnail"] is not "":
                            thumb = item["thumbnail"]
                        listitem = self._create(
                            [item["file"], item["label"] + "  >", "", {"icon": "DefaultFolder.png", "thumb": thumb}])
                        listings.append(self._get_icon_overrides(tree, listitem, ""))
                    else:
        ...
        if selectedItem == -2:
            ...
            return self._explorer_install_widget_provider(history, history[len(history) - 1], label, thumbnail,
                                                          itemType, isWidget)
            ...
            if selectedAction == "::CREATE::":
                ...
                listitem = xbmcgui.ListItem(label=label[len(label) - 1].replace("  >", ""), label2=localItemType,
                                            iconImage="DefaultShortcut.png",
                                            thumbnailImage=thumbnail[len(thumbnail) - 1])
    ...
    def _select_install_widget_provider(self, group, grouping, custom, showNone, currentAction):
        ...
        # Return to where we were
        return self.selectShortcut(group=group, grouping=grouping, custom=custom, showNone=showNone,
    ...
    def _allow_install_widget_provider(self, location, isWidget, nodeAllows=None):
        ...
        if location is not None and (
                "script.skin.helper.service" not in location or self.skinhelperWidgetInstall == False):
            return False
    ...
    def _sourcelink_choice(self, selectedShortcut):
        ...
        if windowID == "Videos":
            ...
            userChoice = dialog.select(LANGUAGE(32078),
                                       [LANGUAGE(32079), LANGUAGE(32015), LANGUAGE(32016), LANGUAGE(32018),
                                        LANGUAGE(32081), LANGUAGE(32082), LANGUAGE(32083)])
            if userChoice == -1:
                return None
            elif userChoice == 0:
            ...
            userChoice = dialog.select(LANGUAGE(32078),
                                       [LANGUAGE(32079), xbmc.getLocalizedString(134), xbmc.getLocalizedString(132),
                                        xbmc.getLocalizedString(20395), LANGUAGE(32084), LANGUAGE(32085),
                                        LANGUAGE(32086)])
            if userChoice == -1:
                return None
            elif userChoice == 0:
            ...
            userChoice = dialog.select(LANGUAGE(32078), [LANGUAGE(32079), xbmc.getLocalizedString(108), "%s (%s)" % (
            xbmc.getLocalizedString(108), xbmc.getLocalizedString(590)), xbmc.getLocalizedString(361), "%s (%s)" % (
                                                         xbmc.getLocalizedString(361), xbmc.getLocalizedString(590))])
            if userChoice == -1:
                return None
            elif userChoice == 0:
                # Escape any backslashes (Windows fix)
    ...
    def _delete_playlist(self, target):
        ...
        if target.startswith("ActivateWindow("):
            try:
                ...
                if len(elements) > 1:
                    if elements[1].startswith("special://profile/addon_data/" + ADDONID + "/") and elements[1].endswith(
                            ".xsp"):
    ...
    def getImagesFromVfsPath(self, path):
        ...
        images = []
        json_query = xbmc.executeJSONRPC(
            '{ "jsonrpc": "2.0", "id": 0, "method": "Files.GetDirectory", "params": { "properties": ["title", "art", "file", "fanart"], "directory": "' + path + '", "media": "files" } }')
        json_query = unicode(json_query, 'utf-8', errors='ignore')
        json_response = simplejson.loads(json_query)
        if json_response.has_key('result') and json_response['result'].has_key('files') and json_response['result'][
            'files']:
    ...
    def selectShortcut(self, group="", custom=False, availableShortcuts=None, windowTitle=None, showNone=False,
        ...
        if showNone is not False and group == "":
            availableShortcuts.insert(0,
                                      self._create(["::NONE::", LANGUAGE(32053), "", {"icon": "DefaultAddonNone.png"}]))
        ...
        if number != -1:
            ...
            if path.startswith("::BACK::"):
                ...
                return self.selectShortcut(group=newGroup, grouping=grouping, custom=custom, showNone=showNone,
                                           currentAction=currentAction)
            if path.startswith("||NODE||"):
                ...
                selectedShortcut = self.explorer(["plugin://" + path.replace("||BROWSE||", "")],
                                                 "plugin://" + path.replace("||BROWSE||", ""),
                                                 [selectedShortcut.getLabel()],
                                                 [selectedShortcut.getProperty("thumbnail")],
                                                 selectedShortcut.getProperty("shortcutType"), isWidget=isWidget)
                ...
                # Video node
                selectedShortcut = self.explorer([path.replace("||VIDEO||", "")], path.replace("||VIDEO||", ""),
                                                 [selectedShortcut.getLabel()],
                                                 [selectedShortcut.getProperty("thumbnail")], "32014",
                                                 isWidget=isWidget)
                # Convert backslashes to double-backslashes (windows fix)
                if selectedShortcut is not None:
                    newAction = selectedShortcut.getProperty("Path")
                ...
                # Audio node
                selectedShortcut = self.explorer([path.replace("||AUDIO||", "")], path.replace("||AUDIO||", ""),
                                                 [selectedShortcut.getLabel()],
                                                 [selectedShortcut.getProperty("thumbnail")], "32019",
                                                 isWidget=isWidget)
                # Convert backslashes to double-backslashes (windows fix)
                if selectedShortcut is not None:
                    newAction = selectedShortcut.getProperty("Path")
                ...
                selectedShortcut = self.explorer(["upnp://"], "upnp://", [selectedShortcut.getLabel()],
                                                 [selectedShortcut.getProperty("thumbnail")],
                                                 selectedShortcut.getProperty("shortcutType"), isWidget=isWidget)
                path = selectedShortcut.getProperty("Path")
            elif path.startswith("||SOURCE||"):
                selectedShortcut = self.explorer([path.replace("||SOURCE||", "")], path.replace("||SOURCE||", ""),
                                                 [selectedShortcut.getLabel()],
                                                 [selectedShortcut.getProperty("thumbnail")],
                                                 selectedShortcut.getProperty("shortcutType"), isWidget=isWidget)
                ...
                if isWidget:
                    ...
                    dialog = xbmcgui.Dialog()
                    userchoice = dialog.yesno(LANGUAGE(32040), LANGUAGE(32060), "", "", LANGUAGE(32061),
                                              LANGUAGE(32062))
                    # False: Display
                    # True: Play
                    if not userchoice:
                    ...
                    userchoice = dialog.select(LANGUAGE(32060),
                                               [LANGUAGE(32061), LANGUAGE(32062), xbmc.getLocalizedString(589)])
                    # 0 - Display
                    # 1 - Play
                    # 2 - Party mode
            ...
            if selectedShortcut is not None and selectedShortcut.getProperty("path") == "::UP::":
                return self.selectShortcut(group=group, custom=custom, availableShortcuts=None, windowTitle=windowTitle,
                                           showNone=showNone, grouping=grouping, currentAction=currentAction)
